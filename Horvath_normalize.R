...
# normalize beta values for use in Horvath model

# load the BMIQ functions by Horvath. I paste them here

#==================== start of Horvath functions =============================#
...
#==================== end of Horvath functions =============================#
select_rows<-function(betas,usedCpGs)
{
	missingCpGs<-setdiff(usedCpGs,rownames(betas))
	if(length(missingCpGs) < 1 ) { # no missing CpGs
		return(betas[usedCpGs,]) 
	}
	...
	selectedBetas<-betas[intersect(usedCpGs,rownames(betas)),]
	selectedBetas<-rbind(selectedBetas,missingRows)
	selectedBetas<-selectedBetas[usedCpGs,] # reorder rows, VERY important
...
{
     requiredParams<-c("betaFile","probeAnnotation21kdat","outFile")
	 ...
	 missingParams<-setdiff(tolower(requiredParams), names(params))
	 if(length(missingParams) > 0) {
	 stop("The following parameters are missing in input:\n", paste(missingParams,collapse=", ")) }
	 if(is.null(params$fastimputation)) { params$fastimputation<-F }
	 else { params$fastimputation<-as.logical(params$fastimputation) }
	 params$fieldsep<-ifelse(is.null(params$fieldsep), ",", params$fieldsep)
	 return(params)
}
...
{
	message("
	Usage: Horvath_normalize.R <config-file>
	
	This program normalizes the illumina 450K or 27K array data for
	use in DNAge prediction, using Horvath's method.
	
	<config-file> contains the input information with two columns
	separated by tab. The 1st column is parameter names and the 2nd
	is the value. The following parameters are recognized:
	
	betaFile:  the file containing beta values, with one sample per
	column. The file can be gzipped.
	
	fieldSep: the field separator in the betafile. The default is
	comma.
	
	probeAnnotation21kdat: the path to Horvath's file 
	probeAnnotation21kdatMethUsed.csv.
	
	outFile: the file for storing results. If ending with '.gz',
	the file will be gzipped.
	
	fastImputation: if 'TRUE', the imputation of missing values will
	be done in the fast mode. The default is 'FALSE'
	")
}
...
library(WGCNA)
library(impute)
# restrict probes to the probeAnnotation21kdatMethUsed files
probeAnnotation21kdatMethUsed<-read.csv(params$probeannotation21kdat, stringsAsFactors=F)
stopifnot(is.character(probeAnnotation21kdatMethUsed$Name))
betas<-betas[probeAnnotation21kdatMethUsed$Name,]

#STEP 2: Imputing
message("# Imputing missing values")
noMissingPerSample<-apply(as.matrix(is.na(betas)),2,sum)
maxMissing<-max(noMissingPerSample,na.rm=TRUE)
nSamples<-ncol(betas)
fastImputation<-params$fastimputation

if (!fastImputation && nSamples>1 && maxMissing<3000 && maxMissing > 0)
{
	# run the following code if there is at least one missing
	dimnames1=dimnames(betas)
	betas <- data.frame(impute.knn(as.matrix(betas))$data)
	dimnames(betas)=dimnames1
} # end of if (! fastImputation )
normalizeData<-T
if ( maxMissing >=3000 ) fastImputation=TRUE

if ( fastImputation || nSamples==1 )
{
	if ( maxMissing >= 3000 ) {
		normalizeData=FALSE
		message("No normalization was performed because of too many missing CpGs\n",
			"Max missing per sample is ", maxMissing)
		}
	
	# run the following code if there is at least one missing
	if( maxMissing >0 & maxMissing < 3000 )
	{
		dimnames1=dimnames(betas)
		for (i in which(noMissingPerSample>0) )
		{
			selectMissing1=is.na(betas[,i]) # the missing CpGs in this sample i
			betas[selectMissing1,i] = as.numeric(probeAnnotation21kdatMethUsed$goldstandard2[selectMissing1])
		} # end of for loop
		dimnames(betas)=dimnames1
	}# end of if
} # end of if (! fastImputation )
...
if (normalizeData)
{
	betas<-t(BMIQcalibration(datM=t(betas),goldstandard.beta= probeAnnotation21kdatMethUsed$goldstandard2,plots=FALSE))
}

out<-params$outfile
if(grepl("\\.gz$", out, ignore.case=T)) {
	zz<-gzfile(out,"w") 
}else{
	zz<-file(out,"w")
}
